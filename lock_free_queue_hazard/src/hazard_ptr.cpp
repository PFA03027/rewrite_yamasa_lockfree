...
public:
	HazardRoot()
	  : hazard_record_head_( nullptr )
	  , hazard_bucket_head_( nullptr )
	{
	}
...
void HazardRoot::deallocateRecord( HazardRecord* record )
{
	assert( record->buckets_in_use == 0 );
...
void HazardRoot::flushRetired( HazardRecord* record )
{
	try {
...
bool HazardRoot::scanHp( ScanedSet& scaned )
{
...
void HazardRoot::deleteItems( const ScanedSet& scaned, RetiredItems& retired )
{
...
void HazardRoot::deleteAllItems( RetiredItems& retired )
{
...
void HazardRecord::clearLocalRecord()
{
	HazardRecord* record = local_record;
	if ( record ) {
...
void HazardRecord::addRetired( void* obj, deleter_func del )
{
	if ( !obj ) return;
