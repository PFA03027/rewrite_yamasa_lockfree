 ...
 */
template <typename T>
inline bool atomic_compare_and_set_wrap(
	volatile std::atomic<T>* obj, T expected_arg, T desired )
{
...
public:
	...
	explicit hazard_ptr( hazard_group<N>& hg )
	  : hazard_record_( hg.hazard_record_ )
	  , hp_( hg.nextHp() )
	  , ptr_( nullptr )
	{
	}
     ...
     * アトミックポインタ変数からの読み出しにおいては memory_order_acquire
     * 相当の メモリバリア効果を持つ。
     */
     ...
     * ある hazard_ptr が保持しているポインタ値を別の hazard_ptr
     * に受け渡す目的で このメソッドを使用してはならない。代わりに swap()
     * を使用すること。
     */
	void reset( T* p )
	{
     ...
     * C++0xのメモリモデルについて正しく理解しており、seq_cst
     * メモリバリア無しでも
	...
	void reset_without_fence( T* p )
	{
		std::atomic_store_explicit(
			hp_, static_cast<const void*>( p ), std::memory_order_release );
		ptr_ = p;
	}
