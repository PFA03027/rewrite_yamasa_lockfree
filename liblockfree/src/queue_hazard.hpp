...
struct QueueBase {
	...
	void enqueue( NodeBase* node )
	{
		...
		for ( ;; ) {
			...
			NodeBase* next = std::atomic_load_explicit( &tail_hp->next, std::memory_order_acquire );
			if ( next ) {
				hazard::detail::atomic_compare_and_set_wrap( &tail_, tail_hp.get(), next );
				continue;
			}
			if ( hazard::detail::atomic_compare_and_set_wrap( &tail_hp->next, next, node ) ) {
				hazard::detail::atomic_compare_and_set_wrap( &tail_, tail_hp.get(), node );
				return;
			}
		}
	}
	              ...
	              hazard::hazard_ptr<NodeBase>& next_hp )
	{
		for ( ;; ) {
			...
			NodeBase* next = std::atomic_load_explicit( &head_hp->next, std::memory_order_acquire );
			if ( !next )
				return false;
			NodeBase* tail = std::atomic_load_explicit( &tail_, std::memory_order_relaxed );
			if ( head_hp.get() == tail )
				hazard::detail::atomic_compare_and_set_wrap( &tail_, tail, next );

			// ここで next をハザードポインタに保持しておくことにより、
			// head_ に対するCASの後でも安全に next の指すオブジェクトにアクセスできる。
			next_hp.reset_without_fence( next );
			if ( hazard::detail::atomic_compare_and_set_wrap( &head_, head_hp.get(), next ) )
				return true;
		}
	}
};
